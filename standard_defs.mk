...
analyze_fossa:
		if [ -z "$(CHANGE_BRANCH)" ]; then \
			$(TOOL) -e FOSSA_API_KEY blockchaintp/fossa:latest fossa analyze \
				--verbose --no-ansi -b ${BRANCH_NAME}; \
			$(TOOL) -e FOSSA_API_KEY blockchaintp/fossa:latest fossa test \
				--verbose --no-ansi -b ${BRANCH_NAME}; \
		else \
			$(TOOL) -e FOSSA_API_KEY blockchaintp/fossa:latest fossa analyze \
				--verbose --no-ansi -b ${CHANGE_BRANCH}; \
			$(TOOL) -e FOSSA_API_KEY blockchaintp/fossa:latest fossa test \
				--verbose --no-ansi -b ${CHANGE_BRANCH}; \
		fi
...
TOOL = docker run --rm -v $(PWD):/project -w /project \
	-v mvn_$(ISOLATION_ID):/root/.m2/repository \
	-v go_$(ISOLATION_ID):/go
...
$(MARKERS)/build_toolchain_docker: $(MARKERS)
	if [ -r docker/toolchain.docker ]; then \
		...
		docker volume create go_$(ISOLATION_ID); \
		docker volume create mvn_$(ISOLATION_ID); \
	fi
	touch $@

.PHONY: clean_toolchain_docker
clean_toolchain_docker:
	docker rmi -f toolchain:$(ISOLATION_ID)
	docker volume rm -f go_$(ISOLATION_ID)
	docker volume rm -f mvn_$(ISOLATION_ID)
